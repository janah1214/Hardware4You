...
namespace Hardware4You.Data
{
    public class UserAuthenticationService
    {
        ...
        public UserAuthenticationService(ProtectedLocalStorage protectedLocalStorage)
        {
            _protectedLocalStorage = protectedLocalStorage;
        }

        public UserAuthenticate? LookupUserInDatabase(string username, string password)
        {
            //TODO: bind dbo.User dont use local storage
            var usersFromDatabase = new List<UserAuthenticate>()
            {
                new()
                {
                    Username = "Admin",
        ...
        public async Task PersistUserToBrowserAsync(UserAuthenticate user)
        {
            string userJson = JsonConvert.SerializeObject(user);
            await _protectedLocalStorage.SetAsync(_userStorageKey, userJson);
        }

        public async Task<UserAuthenticate?> FetchUserFromBrowserAsync()
        {
            try
            {
                ...
                {
                    var user = JsonConvert.DeserializeObject<UserAuthenticate>(storedUserResult.Value);
